...
@section('content')
  <div class="d-flex align-items-end justify-content-between gap-3 mb-3">
    <div>
      ...
      <div class="muted">Kelola periode tanam, status, dan set periode aktif.</div>
    </div>
...
<script>
  ...
  function render(){
    ...
    if (!rows.length){
      tbody.innerHTML = `<tr><td colspan="5" class="text-center muted py-4">Belum ada periode.</td></tr>`;
      return;
    }

    tbody.innerHTML = rows.map(p => {
      const aktif = (Number(p.is_active) === 1) ? `<span class="badge text-bg-success">aktif</span>` : `<span class="badge text-bg-secondary">-</span>`;
  ...
  tbody.addEventListener('click', async (e) => {
    ...
    if (act === 'del'){
      ...
      try{ await apiPost({ action:'delete', id }); await load(); }catch(err){ alert(err.message); }
    }

    if (act === 'active'){
      try{ await apiPost({ action:'set_active', id }); await load(); }catch(err){ alert(err.message); }
    }
  });
